...
class ServiceFinder:
    ...
    def start(self):
        ...
        self._sock = socket.socket(socket.AF_INET, socket.SOCK_DGRAM)
        self._sock.setsockopt(socket.SOL_SOCKET, socket.SO_REUSEADDR, 1)
        # E1: Retry bind with brief delay if ADDRINUSE
        _bind_attempts = 0
        while _bind_attempts < 3:
            try:
                ...
                if _bind_attempts >= 3:
                    ...
                    self._sock = socket.socket(socket.AF_INET, socket.SOCK_DGRAM)
                    self._sock.bind(("", 0))
                    print(f"[lan-discovery] Could not bind to port {DISCOVERY_PORT} after 3 attempts, using ephemeral port")
                else:
                    time.sleep(0.5)
