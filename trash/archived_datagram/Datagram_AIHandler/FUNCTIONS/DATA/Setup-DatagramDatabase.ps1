    ...
    switch ($DatabaseType) {
        'SQLite' {
            ...
            if ($assemblyLoaded -and $SchemaDefinition) {
                try {
                    # Use appropriate SQLite connection via try/catch type detection
                    try {
                        ...
                        $connection.Close()
                        Write-Host "Schema applied successfully (Microsoft.Data.Sqlite)." -ForegroundColor Green
                    } catch {
                        ...
                        try {
                            ...
                            $command.ExecuteNonQuery() | Out-Null
                            $connection.Close()
                            Write-Host "Schema applied successfully (System.Data.SQLite)." -ForegroundColor Green
                        } catch {
                            Write-Warning "Cannot apply schema: SQLite connection type not found. $_"
                        }
                    }
                } catch {
